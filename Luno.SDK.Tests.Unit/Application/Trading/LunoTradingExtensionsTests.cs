...
public class LunoTradingExtensionsTests
{
    ...
    public async Task TradingClient_CalculateOrderSizeAsync_DispatchesQuery()
    {
        ...
        dispatcherMock
            .Setup(d => d.DispatchAsync<CalculateOrderSizeQuery, Task<OrderQuote>>(query, It.IsAny<CancellationToken>()))
            .ReturnsAsync(quote);
        ...
        dispatcherMock.Verify(d => d.DispatchAsync<CalculateOrderSizeQuery, Task<OrderQuote>>(query, It.IsAny<CancellationToken>()), Times.Once);
    }
}
