...
During implementation, the use of a unified `IPipelineBehavior` for both scalar `Task<T>` and streaming `IAsyncEnumerable<T>` operations required `dynamic` dispatch. This approach is incompatible with Native AOT compilation. To ensure Native AOT support, the implementation was modified to strictly segregate pipeline behaviors into `IPipelineBehavior<TRequest, TResponse>` and `IStreamPipelineBehavior<TRequest, TResponse>`. This allows handlers to be instrumented with telemetry and logging without relying on dynamic runtime evaluation.
